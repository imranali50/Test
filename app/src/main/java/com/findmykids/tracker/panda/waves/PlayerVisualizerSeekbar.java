...
public class PlayerVisualizerSeekbar extends AppCompatSeekBar {
    ...
    private void init() {
        ...
        playedStatePainting.setColor(ContextCompat.getColor(getContext(), R.color.white));
        notPlayedStatePainting.setStrokeWidth(1f);
        notPlayedStatePainting.setAntiAlias(true);
        notPlayedStatePainting.setColor(ContextCompat.getColor(getContext(), R.color.gradiant1));
    }
        ...
        for (int a = 0; a < samplesCount; a++) {
            ...
            for (int b = 0; b < drawBarCount; b++) {
                ...
                if (x < denseness && x + dp(2) < denseness) {
                    ...
                    canvas.drawRoundRect(left, top, right, bottom - top, 20, 20, notPlayedStatePainting);

                } else {
                    canvas.drawRoundRect(left, top, right, bottom - top, 20, 20, playedStatePainting);
                    if (x < denseness) {
                        canvas.drawRoundRect(left, top, right, bottom - top, 20, 20, notPlayedStatePainting);
                    }
                }
